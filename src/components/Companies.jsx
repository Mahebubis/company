...
// // Full company list: real API, resizable columns, per-column filters,
// // column-header sort, pagination up to 500/page, login CTA
// // ─────────────────────────────────────────────────────────────────────────────
// import { useEffect, useState, useRef, useCallback, useMemo } from 'react';
// import { useNavigate } from 'react-router-dom';
// import {
//   Building2, Search, RefreshCw, X, ChevronDown, ChevronUp,
//   ChevronsUpDown, LogIn, Globe, ShieldCheck, Shield, Mail,
...
// // ─────────────────────────────────────────────────────────────────────────────
// // COLUMN FILTER DROPDOWN  (reusable)
// // ─────────────────────────────────────────────────────────────────────────────
...
//               className="w-full text-left px-3 py-1.5 text-[11px] text-slate-400
//                          rounded-lg hover:bg-slate-50">
//               — All —
//             </button>
//             {options.map(opt => (
//               <button key={opt} onClick={() => { onChange(opt); setOpen(false); }}
//                 className={`w-full text-left px-2 py-1.5 rounded-lg hover:bg-indigo-50
...
//     no: 44, name: 200, contact: 200, status: 90, verified: 90,
//     industry: 210, jobs: 80, registered: 130, website: 100,
//     tokens: 90, login: 90,
//   });
...
//   const resetAll = () => {
//     setSearch(''); setFilters({ status: '', is_email_verified: '', industry: '', industry_type: '' });
//     setPage(1);
//   };
...
//               <ColHeader col="name" label="Company Name" sortKey="employer_name">
//                 <input value={search}
...
//                     <p className="text-slate-800 text-sm font-medium truncate">{c.email}</p>
...
//                     <div className="flex flex-col gap-1">
//                       {c.industry
//                         ? <Badge label={c.industry} colorKey={INDUSTRY_COLORS[c.industry] ?? 'gray'} />
//                         : <span className="text-slate-300 text-[11px]">—</span>
//                       }
//                       {c.industry_type
//                         ? <Badge label={c.industry_type} colorKey={TYPE_COLORS[c.industry_type] ?? 'gray'} />
//                         : null
//                       }
//                     </div>
//                   </td>

//                   {/* Job count — clickable → /job-posting?employer_id=X */}
//                   <td className="px-3 py-2.5 text-center" style={{ width: colW.jobs }}>
...
//                         <p className="text-slate-400 text-[10px]">{reg.time}</p>
//                       </>
...
//                       <span className="text-sm font-black text-slate-800">{c.hiring_tokens}</span>
//                       <div className="w-14 h-1.5 bg-slate-200 rounded-full overflow-hidden">
//                         <div className="h-full rounded-full"
//                           style={{
//                             width: `${Math.min(100, c.hiring_tokens)}%`,
//                             background: c.hiring_tokens > 50
...
//                   {/* Login — sticky right */}
//                   <td className="px-3 py-2.5 text-center sticky right-0
//                                  shadow-[-6px_0_12px_rgba(0,0,0,0.06)]"
//                     style={{ width: colW.login, background: rowBg }}>
//                     <a href={`https://hire.internshipstudio.com?risky_login=${c.employer_id}`}
...
//           Showing{' '}
//           <strong>{pagination.total === 0 ? 0 : (page - 1) * perPage + 1}</strong>
//           {' – '}
...
//             className="px-2 py-1 rounded-lg text-xs border border-slate-200 text-slate-500
//                        disabled:opacity-30 hover:bg-slate-50">«</button>
//           <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
//             className="px-2.5 py-1 rounded-lg text-xs border border-slate-200 text-slate-500
//                        disabled:opacity-30 hover:bg-slate-50">‹</button>
...
export default function Companies() {
  ...
  const [colW, setColW] = useState({
    ...
    lastlogin: 120, website: 90, tokens: 90, features: 160,
    email: 80, login: 90,
  });
  const resizing = useRef(null);
  const startX = useRef(0);
